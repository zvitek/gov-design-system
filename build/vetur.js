...
const govDir = path.resolve(__dirname, '../docs/pages/components')

let tags = {}
let attributes = {}

fs.readdirSync(govDir, { withFileTypes: true })
    .filter(item => item.isDirectory()
    ).forEach(dir => {
        const componentName = dir.name
        const componentJsPath = govDir + '/' + dir.name + '/api/' + dir.name + '.js'
        console.log('Processing --> ' + componentJsPath)

        let mainTag = {
            "attributes": [],
            "description": `Gov component <b-${componentName}>`,
            "subtags": []
        }
        ...
        componentJs.forEach((tag, index) => {
            ...
            if (index == 0) {
                ...
                curTag = {
                    "attributes": [],
                    "description": `Gov component <${htmlName}>`,
                    "subtags": []
                }
                tags[htmlName] = curTag;
